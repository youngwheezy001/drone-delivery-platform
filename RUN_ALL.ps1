...
Start-Process powershell.exe -ArgumentList "-NoExit -Command `"cd seller-app; npx expo start`""

# 4. Start Customer App
Write-Host "[4/5] Booting Customer App..." -ForegroundColor Green
Start-Process powershell.exe -ArgumentList "-NoExit -Command `"cd customer-app; npx expo start`""

# 5. Start Operator App
Write-Host "[5/5] Booting Ground Crew Operator App..." -ForegroundColor Green
Start-Process powershell.exe -ArgumentList "-NoExit -Command `"cd operator-app; npx expo start`""
